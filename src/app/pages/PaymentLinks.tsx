...
const iconMap: Record<string, any> = {
  IconChart: Icons.IconChart, IconSettings: Icons.IconSettings, IconPlus: Icons.IconPlus,
  IconCheck: Icons.IconCheck, IconTrash: Icons.IconTrash,
};

export function PaymentLinks() {
  const { data, loading, error, refetch } = useAsync(() => configService.getPaymentLinks(), []);
  const [qrFor, setQrFor] = useState<any>(null);
  const [actionsFor, setActionsFor] = useState<string | null>(null);

  if (error) return <ErrorState message={`Couldn't load payment links — ${error.message}`} onRetry={refetch} />;
  if (loading || !data) return <PageLoader label="Loading links" />;

  const { header, stats, rowActions, links } = data;
    ...
    <div style={{ animation: 'payze-fadein 0.4s ease-out' }}>
      ...
      <Card padded={false}>
        <div style={{ padding: '12px 24px', display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 0.8fr 1fr', gap: '16px', background: colors.bg, fontSize: '10px', fontWeight: 500, color: colors.text3, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
          <div>Link</div><div>Merchant</div><div>Amount</div><div>Uses</div><div>Status</div>
...
function QRModal({ link, onClose }: { link: any; onClose: () => void }) {
  return (
