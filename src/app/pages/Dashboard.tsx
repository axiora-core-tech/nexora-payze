...
export function Dashboard() {
  return (
    <div className="w-full max-w-5xl mx-auto space-y-16 mt-8 relative">
      ...
      {/* Header & Balance Block - Creative typography */}
      <section className="flex flex-col md:flex-row items-end justify-between gap-8 pt-8">
        <div>
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-6xl md:text-8xl font-light tracking-tighter text-stone-900">$24,950.</span>
            <span className="text-3xl md:text-5xl font-light text-stone-400">00</span>
          </div>
          <p className="text-stone-500 mt-4 text-sm md:text-base max-w-sm leading-relaxed">
            Good morning, Alex. You received 4 new payments since your last visit.
          </p>
        </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <motion.div whileHover={{ y: -4 }} className="bg-gradient-to-br from-white to-stone-50 p-8 rounded-[40px] border border-stone-100/50 shadow-[0_8px_30px_rgb(0,0,0,0.03)] relative overflow-hidden group">
          ...
          <Link to="/qr" className="flex items-center gap-2 text-stone-800 font-medium text-sm group/btn w-fit">
      ...
      {/* Transaction History - Redesigned as soft floating rows */}
      <section className="pb-16">
        ...
        <div className="space-y-4">
          {[
            { id: 1, name: "Stripe Payout", amount: "+$4,250.00", date: "Today, 10:24 AM", icon: ArrowDownLeft, bg: "bg-emerald-50", color: "text-emerald-600" },
            { id: 2, name: "Apple Subscription", amount: "-$14.99", date: "Yesterday", icon: ArrowUpRight, bg: "bg-rose-50", color: "text-rose-600" },
            { id: 3, name: "Creative Agency Invoice", amount: "+$12,000.00", date: "Oct 12, 2026", icon: ArrowDownLeft, bg: "bg-emerald-50", color: "text-emerald-600" },
            { id: 4, name: "Amazon Web Services", amount: "-$142.00", date: "Oct 10, 2026", icon: ArrowUpRight, bg: "bg-rose-50", color: "text-rose-600" },
          ].map((tx) => (
            <motion.div 
              key={tx.id} 
            ...
            >
              <div className="flex items-center gap-5">
                <div className={`w-12 h-12 rounded-full ${tx.bg} ${tx.color} flex items-center justify-center`}>
              ...
              <div className={`text-xl font-medium tracking-tight ${tx.amount.startsWith('+') ? 'text-stone-800' : 'text-stone-500'}`}>
                {tx.amount}
              </div>
            </motion.div>
          ))}
