...
export function Onboarding() {
  ...
  if (loading || !data) return <PageLoader label="Loading onboarding wizard" />;

  const { header, steps, colors: colorOptions, businessTypes, industries, settlementCadences, plans, webhookEvents } = data;
      ...
      <Card padded style={{ padding: '32px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <Field label="Business type">
              <select style={inputStyle} value={businessType} onChange={e => setBusinessType(e.target.value)}>
                {businessTypes.map((t: string) => <option key={t}>{t}</option>)}
              </select>
            </Field>
            <Field label="Industry">
              <select style={inputStyle} value={industry} onChange={e => setIndustry(e.target.value)}>
                {industries.map((t: string) => <option key={t}>{t}</option>)}
              </select>
            </Field>
            <Field label="Website" style={{ gridColumn: '1 / -1' }} required>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
                {plans.map((p: any) => (
                  <button key={p.id} onClick={() => setPlan(p.id as any)} style={{
                    padding: '16px', background: plan === p.id ? colors.bg : colors.card,
                    border: `${plan === p.id ? '1.5px' : '0.5px'} solid ${plan === p.id ? colors.teal : colors.border}`,
                    borderRadius: radius.md, textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit',
                  }}>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: colors.ink, marginBottom: '4px' }}>{p.label}</div>
                    <div style={{ fontSize: '11px', color: colors.text2 }}>{p.price}</div>
                  </button>
                ))}
              </div>
            </Field>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <Field label="Settlement cadence" style={{ gridColumn: '1 / -1' }}>
              <select style={inputStyle} value={cadence} onChange={e => setCadence(e.target.value)}>
                {settlementCadences.map((c: string) => <option key={c}>{c}</option>)}
              </select>
            </Field>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '16px' }}>
            ...
            <div style={{ padding: '16px', background: colors.bg, borderRadius: radius.md }}>
              ...
              {webhookEvents.map((evt: any) => (
                <label key={evt.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '6px 0', fontSize: '12px', color: colors.ink, fontFamily: typography.family.mono }}>
                  <input type="checkbox" defaultChecked={evt.defaultOn} />
                  {evt.id}
                </label>
              ))}
            </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '28px', paddingTop: '20px', borderTop: `0.5px solid ${colors.border}` }}>
          ...
          {current < steps.length - 1 ? (
            <Button variant="primary" onClick={() => setCurrent(current + 1)} disabled={!canContinue()}>Continue →</Button>
          ) : (
